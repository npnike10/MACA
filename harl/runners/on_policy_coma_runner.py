...
class OnPolicyComaRunner(OnPolicyMARunner):
    ...
    def init_train(self, sacred_run, console_logger):
        ...
        self.critic_buffer = CRITIC_BUFFER_REGISTRY[f"{self.state_type}_coma"](
            {**self.algo_args["train"], **self.algo_args["model"], **self.algo_args["algo"]},
            share_observation_space,
        ...
        if self.algo_args["train"].get("use_valuenorm", True):
            self.value_normalizer = ValueNorm([self.num_agents, 1], device=self.device)
        else:
    ...
    def collect(self, step):
        ...
        value, vq_value, rnn_state_critic = self.critic.get_values(
    ...
    def insert(self, data):
        ...
        (
            ...
            vq_values,  # (n_threads, n_agents, dim)
    ...
    def compute(self):
        ...
        """
        next_value, _, _ = self.critic.get_values(
    ...
    def train(self):
        ...
        # compute advantages
        baselines = self.critic_buffer.vq_value_preds[:-1]
        advantages = self.critic_buffer.returns[:-1] - baselines
